...
const SquareSelector = forwardRef<SquareSelectorRef, Props>(
    function SquareSelector({ image, config, onChange }, ref) {
        ...
        const notifyChange = useCallback((config: SquareConfig, isDragging: boolean) => {
            const now = Date.now();
