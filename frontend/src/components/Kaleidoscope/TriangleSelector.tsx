...
const TriangleSelector = forwardRef<TriangleSelectorRef, Props>(
    function TriangleSelector({ image, config, onChange }, ref) {
        ...
        const notifyChange = useCallback((config: TriangleConfig, isDragging: boolean) => {
            const now = Date.now();
        ...
        return (
            <div ref={containerRef} className="shape-container" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} data-cursor={isDragging ? dragMode : hoverMode}>
                ...
                <img src={image.src} className="image" alt="Selected Image" />
