...
export default function JapanSplitPopulationLassoPage() {
	...
	const processFrame = useCallback(() => {
		...
		const existLasso = lassoPointsRef.current.length >= 3;
		...
		if (existLasso) {
			...
			let sumA = 0;
			let sumB = 0;
			populationData.forEach((dataPoint) => {
				const x = toScreenX(dataPoint.x);
				const y = toScreenY(dataPoint.y);
				if (ctx.isPointInPath(x, y, "evenodd")) {
					sumA += dataPoint.population;
				} else {
					sumB += dataPoint.population;
				}
			});
			setPopulationA(sumA);
			setPopulationB(sumB);
	...
	const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
		...
		lassoPointsRef.current = [...lassoPointsRef.current, { x, y }];
		if (!animationFrameRef.current) {
	...
	return (
		<>
			<h1 className="title">日本分割（人口）（投げ縄選択）</h1>
			<div className="introduction">
				日本を投げ縄選択で分割した時のそれぞれの人口を計算．<br />
				操作方法：ドラッグで領域を選択
			</div>
